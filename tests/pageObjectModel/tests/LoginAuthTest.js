...
fixture("Testing Login page").page`${URLS.LOGIN_URL}`

test("As a user I should be able to see the Login Autorization Page loaded", async (t) => {
  await t
    .expect(URLS.getUrl())
...
test("As a user, I should NOT be able to login if no username and password are provided", async (t) => {
...
test("As a user, I should NOT be able to login if username is provided incomplete without an @", async (t) => {
...
test("As a user, I should NOT be able to login if username is provided with an @ placed in a wrong position", async (t) => {
  await t
    .useRole(WRONG_USER2)
    .expect(LoginPageAuth.invalidEmailAddress.innerText)
...
test("As a user, I should NOT be able to login if username is provided with an @ placed in a wrong position", async (t) => {
  await t
    .useRole(WRONG_USER3)
    .expect(LoginPageAuth.invalidEmailAddress.innerText)
    .contains("Please enter a part following by @. fakeusername@ is incomplete")
})

test("As a user, I should NOT be able to login if password is provided with wrong length", async (t) => {
  LoginPageAuth.submitLoginForm(
...
test("As a user, I should NOT be able to login if no password is provided", async (t) => {
  await t
...
test("As a user, I should NOT be able to login if username is provided with valid email address and no password", async (t) => {
  await t
...
test("As a user, I should NOT be able to login if no username and password are is provided", async (t) => {
  await t.click(LoginPageAuth.loginBtn)
  await t
    .expect(LoginPageAuth.invalidEmailAddress.innerText)
    .contains("Please enter a valid email address.")
...
test("As a user, I should NOT be able to login if username is provided with . in wrong position", async (t) => {
  await t
    .useRole(WRONG_USER5)
    .expect(LoginPageAuth.invalidEmailAddress.innerText)
...
test("As a user, I should NOT be able to login if password is provided with a wrong format", async (t) => {
  await t
    .useRole(WRONG_USER6)
    .expect(LoginPageAuth.invalidEmailAddress.innerText)
    .contains(
...
test("As a unregistered user, I should NOT be able to login by providing invalid username", async (t) => {
  LoginPageAuth.submitLoginForm(
...
test("As a registered user, I should not be able to login by providing invalid password", async (t) => {
  LoginPageAuth.submitLoginForm(
...
test("As a user, I should be able to log in successfully by provinding valid credentials", async (t) => {
  await t
